...
CMD ["sh", "-c", "echo 'Starting E-Commerce application on port ${PORT:-8080}' && ls -la . && java -Dspring.profiles.active=prod -Dserver.port=${PORT:-8080} -Dlogging.level.org.springframework=DEBUG -jar app.jar"]